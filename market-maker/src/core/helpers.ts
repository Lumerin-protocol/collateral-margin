/**
 * Venue-agnostic helpers that don't have a clear home in another module.
 * Mostly used by tests and pre-bootstrap warm-up paths; nothing here is
 * on the hot trading path.
 */
 ...
 * to `modelledOrders`. Orders at the same price offset each other.
 ...
 * "Close" = last observed price in each bucket. Missing buckets are filled
 * with LOCF (last observation carried forward).
 */
export function resampleHourlyClose(prices: TimedPrice[], intervalMs = 60 * 60 * 1000): TimedPrice[] {
