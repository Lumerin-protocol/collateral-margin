...
import type { WalletContext } from "./adapter.ts";

/**
 * Resolves named wallets declared in config into live viem wallet contexts.
 * One `privateKeyToAccount` call per name — shared when multiple venues
 * reference the same wallet name.
 */
export class WalletRegistry {
  private readonly contexts = new Map<string, WalletContext>();

  constructor(walletConfigs: Record<string, { privateKey: Hex }>, chain: Chain, transport: Transport) {
