 ...
 *   3. ln(2) is computed once at the requested precision via the same series at y=1/2.
 */
...
/**
 * ln(y) for y in [1/2, 2] via atanh series:
 *   ln(y) = 2 * Σ u^(2i+1) / (2i+1),  u = (y-1)/(y+1)
 * |u| <= 1/3 here, so it converges quickly.
 */
function atanhSeries(y: Fraction, precisionBits: number): Fraction {
  const u = y.sub(1).div(y.add(1));
...
function ln2Cached(precisionBits: number): Fraction {
  ...
  // ln(2) = -ln(1/2). y = 1/2 stays in [1/2, 2] with k=0.
